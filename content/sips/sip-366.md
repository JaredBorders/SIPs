...
Add the following `escrowedCollateralByAccountPerPool` and `escrowedCollateralByAccount` fields to the `Data` `struct` to store the amount of collateral that is currently escrowed for a given account due to all outstanding intents to delegate or undelegate collateral:

```solidity
mapping(uint128 accountId => mapping(uint128 poolId => uint256 escrowedCollateral) escrowedCollateralByPool) escrowedCollateralByAccountPerPool;
mapping(uint128 accountId => uint256 escrowedCollateral) escrowedCollateralByAccount;
```

> The proposed approach of using four fields to store intent, rather than a single field mapping of an `accountId` to `DelegateCollateralIntent` objects, is more effective. It avoids conflicts that would occur when `DelegateCollateralIntent` objects are updated, as discussed in the article [here](https://medium.com/@0xFluffyBeard/migrating-the-eip-2535-diamond-storage-layout-cb28b9c47e71). Storing intent nonces in an array allows for far less complicated updates to the `DelegateCollateralIntent` object. Moreover, detailed accounting is necessary to avoid errors caused by deliberately malicious declarations of intent.
...
3. Check the validity of the collateral amount to be delegated, considering the `escrowedCollateralByAccount`.
4. Update the `escrowedCollateralByAccount` to reflect the collateral amount declared in the intent for the identified account and pool.
...
7. Delete the intent nonce from `intentNoncesByAccount` for the involved account.
8. Delete the intent from `intentByNonce` using the specific nonce.
9. Update the amount of collateral delegated to the pool according to the intent.
10. Emit a `DelegateCollateralIntentProcessed` event upon successful processing.
11. Execute steps 2 to 11 for every intent ready for processing for the mentioned account, pool, and collateral type.

> The order by which the intents are processed should not matter so long as the `escrowedCollateralByAccount` mapping field is properly maintained.
